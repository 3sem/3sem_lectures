...
Для того, чтобы описать процесс конструктивно, просто опишем в каких состояниях он бывает (и чем они определяются), и какие операции над ним можно совершать.

Процесс: состояния.
==

Количество состояний и их категории отличаются от ОС к ОС. Мало того, они могут отличаться в утилитах мониторинга.

К примеру, в современных линуксах, как правило, 8-9 состояний:
```
...
```
В то время как в утилите top их 5:
...
'T' = traced or stopped -- Получен сигнал приостановки, повис на точке останова в отладчике и т.д.

'Z' = zombie -- З

Состояние "Ожидание" на деле может быть 2 видов -- Interruptable (S in top) и Uninterruptable (D in top). Если вы долго висите в Uninterruptable Sleep, то в вашей ОС наверняка есть баг.

Состояния "Готов" и "Исполняется" в top -- R, они не различаются.
